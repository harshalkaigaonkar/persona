  ...
  return (
    <div
      ...
      className={`group relative overflow-hidden my-2 w-full h-auto bg-base-1 border border-base-3 rounded-md cursor-pointer transition duration-200 hover:-translate-y-2 ${
        showButton ? 'bg-right-top bg-no-repeat ' : ''
      }`}
      onClick={(e: any) => {
      ...
      {!!cta && (
        <>
          <Transition
            show={!!showButton}
            as={React.Fragment}
            enter="transition-all ease-in-out duration-[700] origin-[50%_50%_10px] transform"
            enterFrom="-right-[100%]"
            enterTo="right-4"
            leave="transition-all ease-in-out duration-[500] origin-[50%_50%_10px] transform"
            leaveFrom="right-4"
            leaveTo="-right-[100%]"
          >
            <div className="absolute flex max-w-full top-3.5">
              <div className="w-full h-auto">
                <button
                  name={
                    typeof cta?.content === 'string' ? cta?.content : `icon`
                  }
                  onClick={() => {}}
                  className="inline-flex justify-center items-center gap-1 bg-primary px-4 py-2 text-high rounded-md z-50 shadow-xl"
                >
                  ...
                  {typeof cta?.content === 'string' && (
                    <>
                      <svg
                        ...
                        } group-hover:translate-x-[0.125rem] transition delay-[4000] duration-100`}
                      >
                        {cta?.url?.external ? (
                          // top-right-arrow svg
                          <path
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <div className="w-64 h-64 bg-gradient-radial-purge absolute top-0 right-0 z-10" />
          </Transition>
        </>
      )}
      {children}
